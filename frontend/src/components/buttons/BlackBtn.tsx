...
const BlackBtn: React.FC<ButtonProps> = ({ children, onClick, type = "button" }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      className="px-5.5 py-2.5 relative overflow-hidden bg-black text-white border-2 border-black font-medium tracking-tight rounded-xl transition-all transform-gpu duration-300 ease-[cubic-bezier(0.68,-0.55,0.265,1.55)] hover:-translate-y-[2px] hover:scale-105 hover:bg-(--bamboo-green) hover:border-[var(--bamboo-green)] hover:shadow-[0_8px_20px_rgba(124,179,66,0.4)]"
    >
