...
const WhiteBtn: React.FC<ButtonProps> = ({
  ...
  return (
    ...
    >
      <span className="absolute top-0 left-[-100%] w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-all duration-500 hover:left-full whitespace-nowrap" />
      <span className="relative z-10">{children}</span>
    </button>
  )
}
